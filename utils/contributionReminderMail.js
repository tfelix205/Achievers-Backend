exports.contributionReminderMail = (name, groupName, contributionAmount, dueDate) => {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Reminder: Make Your Contribution - Splita</title>
</head>
        ...
        <table role="presentation" style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="padding:32px 40px;">
              <h2 style="margin:0 0 12px;color:#1a1a1a;font-size:20px;">Ajo Contribution Reminder</h2>
              <p style="margin:0 0 16px;color:#666;font-size:14px;">Hi ${name},</p>
              <p style="margin:0 0 20px;color:#666;font-size:14px;">
                This is a friendly reminder to make your contribution for <strong>${groupName}</strong>.
                The required amount is <strong>₦${contributionAmount}</strong>.
              </p>
              ${dueDate ? `<p style="margin:0 0 20px;color:#666;font-size:14px;">Due date: <strong>${dueDate}</strong></p>` : ''}
              <p style="margin:0 0 24px;color:#666;font-size:14px;">Please make your payment on/before the due date to avoid penalties and keep the cycle moving.</p>
            </td>
          </tr>
          <tr>
