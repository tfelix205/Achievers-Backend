exports.cycleCompletedMail = (name, groupName, totalCollected, cycleStart, cycleEnd) => {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Cycle Completed - Splita</title>
</head>
        ...
        <table role="presentation" style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="padding:32px 40px;">
              <h2 style="margin:0 0 12px;color:#1a1a1a;font-size:20px;">Cycle Completed</h2>
              <p style="margin:0 0 16px;color:#666;font-size:14px;">Hi ${name},</p>
              <p style="margin:0 0 12px;color:#666;font-size:14px;">
                The cycle for <strong>${groupName}</strong> has been completed.
              </p>
              ${totalCollected ? `<p style="margin:0 0 8px;color:#666;font-size:14px;">Total collected: <strong>₦${totalCollected}</strong></p>` : ''}
              ${cycleStart && cycleEnd ? `<p style="margin:0 0 12px;color:#666;font-size:13px;">Period: ${cycleStart} — ${cycleEnd}</p>` : ''}
              <p style="margin:0 0 16px;color:#666;font-size:14px;">Thank you for participating. If you have questions or concerns, contact the Splita support team.</p>
            </td>
          </tr>
          <tr>
